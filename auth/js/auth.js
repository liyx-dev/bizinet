...
(function () {
  ...
  // ---------------------------------------------------------------------
  // OFFLINE DETECTION — never let a user submit into a dead connection
  // without knowing why it's hanging.
  // ---------------------------------------------------------------------
  ...
  // This is the orphan-account fix in practice: call the idempotent RPC,
  // then VERIFY via runtime state. Retry with backoff. Never just hope.
  // ---------------------------------------------------------------------
  async function guaranteeWorkspace(businessName, { maxAttempts = 8 } = {}) {
    let lastError = null;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      // Exponential-ish backoff, capped, so we don't hammer the DB.
  ...
  if (signupForm) {
    signupForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        // No session means email confirmation is required upstream — handle
        // gracefully even though your project currently has it disabled, so
        // this page keeps working if that setting ever changes.
        ...
        // We have a live session — guarantee the workspace exists before
        // navigating anywhere. This is the critical step that prevents
        // orphaned auth-only accounts.
  ...
  if (signinForm) {
    signinForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        // store (e.g. they abandoned signup mid-provisioning days ago,
        // before this page existed), heal it silently on login rather
        // than stranding them on a broken dashboard redirect.
        if (!runtime?.store_id) {
  ...
  async function boot() {
    try {
      ...
      if (!runtime?.store_id) {
        // Same healing path as login: a session exists with no store.
        // We don't know the intended business name at boot time, so we
        // fall back to a placeholder the user can rename in onboarding —
        // the goal here is zero dead-end accounts, not a perfect name.
