...
try {
  ...
  // INTEL: video uploaded to R2
  if (typeof window.INTEL?.trackAction === 'function') window.INTEL.trackAction('videoUploaded');
  }
...
p_discount_percentage: parseFloat(els.discount.value) || 0,
  ...
  // INTEL: product updated
  if (typeof window.INTEL?.trackAction === 'function') window.INTEL.trackAction('productAdded');
    } else {
...
id);
  ...
  window.toggleProductFeatured = async function (id, current) {
    ...
    loadProducts();
  // INTEL: product featured toggled
  if (typeof window.INTEL?.trackAction === 'function') window.INTEL.trackAction('productFeatured');
};
...
  toast("Product deleted. ✓", "success");
  // INTEL: product deleted
  if (typeof window.INTEL?.trackAction === 'function') window.INTEL.trackAction('productDeleted');
  loadProducts();
};
...
style="height:110px;width:100%;margin-top:12px;"></div>`;
  ...
  // FETCH RAW PRODUCTS TABLE AND FILTER STRICTLY BY THE RUNTIME STORE
CONTEXT
...
onclick="moveProduct('${product.id}','up')"> </button>
...
<button class="action-btn"
                                                                          ...
                                                                          ✏️ Edit</button>
onclick="moveProduct('${product.id}','down')"> </button>
...
  const upload = await fetch(result.uploadUrl, {
  ...
  if (!upload.ok) throw new Error("File upload didn't complete. Check your connection and
retry.");
  return result.publicUrl;
}
...
function getImageDimensions(file) {
  return new Promise((resolve) => {
    ...
    img.onload = () => { URL.revokeObjectURL(url); resolve({ width: img.naturalWidth, height:
img.naturalHeight }); };
    ...
    if (grid && !appendMode) {
      grid.innerHTML = `
        ...
        <div style="grid-column:1/-1;text-align:center;padding:48px 20px;">
         ...
         <p style="font-weight:700;color:#1e293b;font-size:15px;margin-bottom:6px;">Connection
is taking too long</p>
         <p style="color:#64748b;font-size:13px;margin-bottom:16px;">Check your network and
try again</p>
  ...
  _injectLoadMoreTrigger();
    ...
    if (grid && !appendMode) {
      grid.innerHTML = `
        ...
        <div style="grid-column:1/-1;text-align:center;padding:48px 20px;">
          <div style="font-size:40px;margin-bottom:12px;"> </div>
          <p style="font-weight:700;color:#1e293b;font-size:15px;margin-bottom:6px;">Couldn't
load your stories</p>
          <p style="color:#64748b;font-size:13px;margin-bottom:16px;">Something went wrong on
our end</p>
          <button onclick="loadStories(false)" style="
           background:linear-gradient(135deg,#28A428,#34BF49);color:#fff;
 ...
 trigger.style.cssText = `
  ...
  if (failed) {
    trigger.innerHTML = `
      ...
      <p style="font-size:13px;font-weight:700;color:#FF7A00;margin:0;text-align:center;">No
connection right now</p>
      <p style="font-size:11px;font-weight:500;color:#94a3b8;margin:0;text-align:center;">We'll
load more stories once you're back online</p>
    ...
    trigger.querySelector("#stLoadMoreRetryBtn").onclick = e => { e.stopPropagation();
_attemptLoadMore(); };
...
        -webkit-mask:radial-gradient(farthest-side,transparent calc(100% - 5px),#000 calc(100% -
5px));
      ...
      <span id="stLoadMoreLabel" style="
        ...
        background-clip:text;animation:stLoadMorePulse 2s ease-in-out
infinite;letter-spacing:.3px;">
        Load more stories
      </span>`;
    trigger.onclick = () => _attemptLoadMore();
  }
...
                                                          ✅
      ...
      <span style="font-size:13px;font-weight:700;color:#28A428;">Back online!
Loading…</span>`;
    }
...
function renderStories() {
 ...
 if (allStories.length > 0) {
   ...
   if (primaryRow.viewer_role === "owner" && ownerPanel && ownerPanel.style.display ===
"none") {
...
     }
 ...
 const filtered = allStories.filter(s => {
   ...
   if (currentCreatedByFilter === "me" && currentUserUidCache && s.creator_id !==
currentUserUidCache) return false;
...
20px;color:#64748b;font-weight:600;">
 ...
 grid.innerHTML = filtered.map((s, idx) => buildStoryCard(s, idx, filtered)).join("");
  ...
  if (storyHasMore && !document.getElementById("storyLoadMoreTrigger") && !keyword &&
      ...
      ${thumbSrc
        ? `<img src="${thumbSrc}" alt="video cover"
            ...
            onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                              ...
                              🎬
           <div class="st-card-video-fallback"

                                                     🎬
style="display:none;"><span> </span><small>Video</small></div>`
...
onerror="this.style.display='none';this.parentElement.querySelector('.st-card-img-fallback').style.

                                                                      🖼
display='flex';">
     <div class="st-card-img-fallback" style="display:none;"><span> </span><small>Image
...
      : s.link_type === "whatsapp"
    ...
    title="${s.link_type === "product" ? (s.product_name || "Product") : s.link_type ===
...
👁
  const ribbonLabel = s.status === "active" ? " Live" : s.status === "expired" ? "
"     Hidden";
  ...
  <button onclick="openStoryModal('${s.id}')"
    style="background:linear-gradient(135deg,#1877F2,#0d5bbf);color:#fff;border:none;
         border-radius:8px;padding:6px
                                                                                ...
                                                                                ✏️
14px;font-size:12px;font-weight:700;cursor:pointer;transition:.2s;"
  ...
  <button onclick="deleteStory('${s.id}',this)"
   style="background:linear-gradient(135deg,#FF3B30,#C1271A);color:#fff;border:none;
        border-radius:8px;padding:6px
                                                                                    ...
                                                                                    🗑
10px;font-size:12px;font-weight:700;cursor:pointer;transition:.2s;"
...
0;position:absolute;top:0;left:0;z-index:3;">${ribbonLabel}</div>
    ${s.is_featured ? `
     <div style="background:linear-gradient(90deg,#FFD700,#FF7A00);color:#111;

                                                                                   ⭐
             font-size:11px;font-weight:700;padding:3px 10px;
             border-radius:0 0 0 8px;position:absolute;top:0;right:0;z-index:3;">
Featured</div>` : ""}
    ${mediaHtml}
...
     <div class="st-card-title">${s.title || "Untitled Story"}</div>
     ...
     <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:8px;align-items:center;">
       <span
style="background:${s.type==='image'?'#e0f2fe':s.type==='video'?'#fce7f3':'#f0fdf4'};
       ...
       ${linkTag}
       ${s.aspect_ratio ? `<span
style="background:#f1f5f9;color:#475569;font-size:11px;font-weight:600;padding:2px
8px;border-radius:20px;">${s.aspect_ratio}</span>` : ""}
     </div>
     <div style="display:inline-flex;align-items:center;gap:6px;padding:5px 10px;
              background:linear-gradient(135deg,#f8fafc,#f1f5f9);
              border-radius:20px;margin-bottom:10px;border:1px solid
#e2e8f0;width:fit-content;">
     ...
     <div
style="display:flex;gap:10px;font-size:12px;color:#64748b;font-weight:600;flex-wrap:wrap;margi
...
                                                 ✏️
 if (editId) {
   if (modalTitle) modalTitle.textContent = " Edit Story";
...
onloadeddata="this.parentElement.parentElement.querySelector('.st-media-network-loader')?.re
move();"
...
oncanplaythrough="this.parentElement.parentElement.querySelector('.st-media-network-loader')
?.remove();"
...
async function _handleStoryMediaChange(e) {
  ...
  e.target.value = "";
    ...
    if (isVideo && file.size > 100 * 1024 * 1024) {
      return toast("Video is too large. Please keep it under 100MB.
    }
...
  if (isImage) {
  ...
  if (storyEditingId) {
    await _syncMediaToSupabase(storyEditingId, mediaUrl, thumbUrl, mediaType, width,
height, aspectRatio, fileSize, duration);
...
    } else {
      await _syncMediaToSupabase(storyDraftId, mediaUrl, thumbUrl, mediaType, width, height,
aspectRatio, fileSize, duration);
    ...
    storyCurrentMedia = { url: mediaUrl, thumbUrl, type: mediaType, width, height, aspectRatio,
fileSize, duration };
...
function _resetCtaType(type) {
  ...
  const map = { product: "ctaProductPanel", whatsapp: "ctaWhatsappPanel", external:
"ctaExternalPanel" };
...
style="font-size:12px;font-weight:600;color:#64748b;">       Search your products:</label>
     <input type="text" id="st_story_product_search" placeholder="Type to filter…"
       ...
       onfocus="this.style.borderColor='#28A428';this.style.boxShadow='0 0 0 3px
rgba(40,164,40,.12)'"
...
    const renderOptions = (filterText = "") => {
      ...
      if (matched.length === 0) { sel.innerHTML = `<option value="">No products found</option>`;
return; }
      matched.forEach(p => {
...
window.toggleFeatured = () => { storyIsFeatured = !storyIsFeatured;
_setFeaturedToggle(storyIsFeatured); };
...
window.saveStory = async function () {
 ...
 try {
   ...
   const productId = (storyCtaType === "product" && productSel?.value) ? productSel.value :
null;
...
      p_cta_text:        ctaText,
    ...
    const { error: e } = await supabaseClient.from("stories").update({
      ...
      auto_delete_at: new Date(Date.now() + (storySelectedHours + 24) * 3600 *
1000).toISOString()
    }).eq("id", storyDraftId);
    error = e;
  ...
  // INTEL: story saved/published
  if (typeof window.INTEL?.trackAction === 'function') window.INTEL.trackAction('storyCreated');
    closeStoryModal();
...
window.toggleStoryVisibility = async function (id, btnEl) {
 ...
 try {
   ...
   if (story) {
     ...
     story.status = isNowHidden ? "hidden" : (new Date(story.expires_at) < new Date() ?
"expired" : "active");
   }
   renderStories();
   updateStats();
...
24px;max-width:340px;width:90%;
          text-align:center;box-shadow:0 25px 60px rgba(0,0,0,.18);animation:stScaleIn .25s
ease;">
...
style="width:100%;height:180px;object-fit:cover;border-radius:12px;margin-bottom:14px;">`
        ...
        ${data.price ? `<p
style="font-weight:700;font-size:18px;color:#28A428;margin-bottom:8px;">${data.price}</p>` :
""}
        ${data.description
        ...
        <div style="background:linear-gradient(135deg,#fff9e6,#fff3cc);
                border:1px solid #FFD700;border-radius:10px;padding:10px
14px;margin-top:8px;">

           🛒
         <p style="font-size:12px;color:#92400e;font-weight:600;margin:0;">
               On the storefront, tapping this story opens <strong>${data.name}</strong>'s page
directly.
         </p>
        </div>`;
    }
...
function _renderPreviewSlide() {
 ...
 if (!s) return;
  ...
  if (track) track.innerHTML = previewStoryList.map((_, i) =>
    `<div class="st-preview-dot ${i === previewIndex ? "active" : i < previewIndex ? "done" :
""}"></div>`
  ).join("");
...
style=\\'width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,#28A428,

                         🏪
#34BF49);display:flex;align-items:center;justify-content:center;font-size:18px;border:2px solid
rgba(255,255,255,.4);\\'> </div>'">`
...
  border:4px solid rgba(255,255,255,.1);border-top-color:#ffffff;
 ...
 if (s.type === "video") {
   ...
   ph.innerHTML = `
    ...
    <p style="color:rgba(255,255,255,.6);font-size:13px;font-weight:700;">${s.title || "Audio
Story"}</p>`;
   wrap.appendChild(ph);
   ...
   _startProgressBar(slideDuration);
  previewTimer = setTimeout(() => { if (!previewCtaPaused && !previewHolding)
previewNav(1); }, slideDuration * 1000);
...
         e.stopPropagation();
      ...
      ctaNew.addEventListener("click", function (e) {
        ...
        _previewPauseForCta();
        window.open(
          `https://wa.me/${s.whatsapp_number}?text=${encodeURIComponent("Hi, I saw your
story: " + (s.title || ""))}`,
          "_blank"
        );
...
      ?.querySelectorAll("video,audio")
   ...
   screen.addEventListener("mouseleave", () => {
     if (previewHolding && !previewCtaPaused) {
       previewHolding = false;
       document.getElementById("previewMediaWrap")
         ?.querySelectorAll("video,audio")
         .forEach(el => { try { el.play().catch(() => {}); } catch (_) {} });
...
window.filterMediaType = function (mediaType, btnElement) {
  ...
  document.querySelectorAll(".st-media-filter-btn").forEach(btn =>
...
style="display:flex;align-items:center;gap:8px;justify-content:center;">
   <span style="display:inline-block;animation:cat-spin .7s linear infinite;">
 </span>`;
...
via string splitting iterator)
  ...
  // INTEL: category updated
  if (typeof window.INTEL?.trackAction === 'function') window.INTEL.trackAction('categoryAdded');
  } else {
...
20px;background:var(--surface-card);border-radius:var(--radius-md);border:1px solid

                                                       😕
rgba(255,59,48,.2);">
      <div style="font-size:36px;margin-bottom:10px;"> </div>
      <p style="font-weight:700;color:var(--text-primary);margin-bottom:6px;">Couldn't load
categories</p>
      <p style="font-size:13px;color:var(--text-muted);margin-bottom:16px;">Check your
connection and try again</p>
      <button onclick="loadCategoriesTab()"
style="background:linear-gradient(135deg,var(--liyog-green),var(--liyog-green-dark));color:#fff;bo
                                                                                 🔄
rder:none;border-radius:var(--radius-sm);padding:9px
20px;font-size:13px;font-weight:700;cursor:pointer;font-family:var(--font-body);">
Retry</button>
      </div>`;
...
async function settingsUploadFile(file, folder) {
  ...
  const res = await fetch(`${supabaseUrl}/functions/v1/generate-r2-upload-url`, {
    ...
    headers: {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${currentSessionToken}` // Authenticates your secure edge
functions
    },
    body: JSON.stringify({
      fileName: file.name,
...
  catBadges.forEach((badge, i) => {
    ...
    chip.innerHTML = `      ${badge} <span class="remove-badge"
onclick="removeBadge(${i})">×</span>`;
...
window.removeLogo = async function() {
 ...
 try {
  ...
  // INTEL: logo removed
  if (typeof window.INTEL?.trackAction === 'function') window.INTEL.trackAction('profileUpdated');
  } catch(e) {
...
function renderStorePhotos() {
 ...
 storePhotos.forEach((photo, i) => {
   ...
   if (photo.isUploading) {
     item.innerHTML = `
       <div
style="display:flex;align-items:center;justify-content:center;height:100%;background:#f1f5f9;">
        <div class="spinner" style="width:24px;height:24px;"></div>
       </div>`;
   } else {
     item.innerHTML = `
       <img src="${photo.url}" alt="Store photo ${i+1}" loading="lazy">
       <button class="st-photo-remove" onclick="removeStorePhoto(${i})"
title="Remove">×</button>`;
    }
...
function initQuill() {
  ...
  bioEditor = new Quill("#st_bio_editor", {
    theme: "snow",
    placeholder: "Tell customers about your store — your story, values, what makes you
special...",
    modules: {
      toolbar: [
        [{ header: [1, 2, 3, false] }],
...
style="width:16px;height:16px;border-width:2px;display:inline-block;margin-right:8px;vertical-alig
n:middle;"></div>Saving…`;
...
  // RESTORED SIGNATURE: Removed p_store_id parameter so it perfectly maps your
...
[contenteditable='true']:focus")) return;
